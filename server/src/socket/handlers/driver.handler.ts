...
export function driverHandlers (
  ...
  socket.on('driver:endTrip', async () => {
    try {
      ...
      // Emit success event to driver (busId and routeId are guaranteed to be strings here)
      if (busId && routeId) {
        socket.emit('trip:ended', { tripId, busId, routeId })
      }
  ...
  socket.on('disconnect', async () => {
    if (socket.currentTripId && socket.currentBusId && socket.currentRouteId) {
      try {
        ...
        console.log(
          `Driver ${socket.userId || 'unknown'} disconnected with active trip ${
            socket.currentTripId
          }`
        )
      } catch (error) {
