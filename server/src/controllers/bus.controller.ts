...
import {
  getBusLocation as getBusLocationFromCache,
  getActiveBus
} from '../config/redis'
...
export const getBusLocation = async (
  ...
  try {
    ...
    const location = await getBusLocationFromCache(id)

    if (!location) {
      res
        .status(404)
        .json({ success: false, error: 'Bus location not available' })
      return
    }
    ...
    res
      .status(500)
      .json({ success: false, error: 'Failed to get bus location' })
  }
}

export const createBus = async (req: Request, res: Response): Promise<void> => {
  try {
    const { plateNumber, name, capacity, routeId, driverId, isActive } =
      req.body
    ...
    if (existingBus) {
      res
        .status(400)
        .json({ success: false, error: 'Plate number already registered' })
      return
    }
...
export const deleteBus = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (activeTripId) {
      res
        .status(400)
        .json({ success: false, error: 'Cannot delete bus with active trip' })
      return
    }
