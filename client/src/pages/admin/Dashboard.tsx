...
export default function AdminDashboard() {
  ...
  const mapCenter: [number, number] =
    lv.length > 0 ? [lv[0].latitude, lv[0].longitude] : [31.5204, 74.3587];
  const adminCount = Math.max(
    0,
    stats.users.total - stats.users.riders - stats.users.drivers,
  );
  const fleetPct =
    stats.buses.total > 0 ? stats.buses.active / stats.buses.total : 0;
  const card = {
    background: "#fff",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      {/* ── 4 stat cards ── */}
      <div className="admin-stat-grid" style={{ gap: 14 }}>
        {[
          {
            label: "Total Users",
            value: stats.users.total,
            sub: `${stats.users.riders} riders · ${stats.users.drivers} drivers`,
            icon: Users,
            accent: C.indigo,
            bg: C.indigoBg,
            link: "/admin/users",
          ...
          },
          {
            ...
            sub: `of ${stats.buses.total} fleet`,
            icon: Bus,
            accent: C.sky,
            bg: C.skyBg,
            live: true,
          ...
          },
          {
            ...
            sub: `of ${stats.routes.total} total`,
            icon: Activity,
            accent: C.indigo,
            bg: C.indigoBg,
            link: "/admin/routes",
      ...
      {/* ── Analytics row ── */}
      <div className="admin-3-grid" style={{ gap: 14 }}>
        ...
        <div style={{ ...card, padding: "20px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
            <DonutChart
              data={[
                { value: stats.users.riders, color: C.sky },
                { value: stats.users.drivers, color: C.indigo },
                { value: adminCount, color: "#cbd5e1" },
              ]}
              size={90}
            />
            <div style={{ flex: 1 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
            ...
            <div style={{ flex: 1 }}>
              {[
                ...
                {
                  label: "Inactive",
                  value: stats.buses.total - stats.buses.active,
                  color: "#cbd5e1",
                },
              ].map(({ label, value, color }) => (
                <div
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
            <div style={{ position: "relative", flexShrink: 0 }}>
              <Ring
                pct={
                  stats.trips.today > 0
                    ? stats.trips.ongoing / stats.trips.today
                    : 0
                }
                color={C.sky}
                size={90}
      ...
      {/* ── Map + Trips ── */}
      <div className="admin-map-grid" style={{ gap: 14 }}>
        <div style={{ ...card, overflow: "hidden" }}>
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ fontSize: 13.5, fontWeight: 700, color: C.dark }}>
                Live Bus Map
              </span>
            </div>
            <span
              style={{
                ...
                padding: "3px 10px",
                borderRadius: 99,
                background: lv.length > 0 ? C.indigoBg : "#f1f5f9",
                color: lv.length > 0 ? C.indigo : C.light,
              }}
            >
              {lv.length > 0 && (
                <span
              ...
              recentTrips.map((trip, i) => (
                <div
                  ...
                  style={{
                    ...
                    padding: "11px 16px",
                    borderBottom:
                      i < recentTrips.length - 1 ? "1px solid #f8fafc" : "none",
                  }}
                >
                  <div
                    style={{
                  ...
                  >
                    <span
                      style={{
                        ...
                        flexShrink: 0,
                        background:
                          trip.status === "ongoing" ? C.sky : "#e2e8f0",
                        boxShadow:
                          trip.status === "ongoing"
                            ? `0 0 5px ${C.sky}`
                            : "none",
                  ...
                  <span
                    style={{
                      ...
                      flexShrink: 0,
                      background:
                        trip.status === "ongoing" ? C.skyBg : "#f1f5f9",
                      color: trip.status === "ongoing" ? C.sky : C.slate,
                    }}
                  >
                    {trip.status}
                  </span>
      ...
      {/* ── Quick links ── */}
      <div className="admin-quick-grid" style={{ gap: 12 }}>
        {[
          {
            to: "/admin/users",
            icon: Users,
            label: "Users & Access",
            desc: "Roles, approvals, accounts",
            accent: C.indigo,
            bg: C.indigoBg,
          },
          {
            ...
            desc: "Build and configure routes",
            accent: C.sky,
            bg: C.skyBg,
          },
          {
            ...
            desc: "Buses, drivers, live status",
            accent: C.indigo,
            bg: C.indigoBg,
          },
