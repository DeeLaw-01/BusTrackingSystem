  ...
  return (
    <div className='max-w-6xl mx-auto space-y-6 pb-12'>
      ...
      <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-4'>
        <div>
          <h1 className="admin-header text-2xl">User Management</h1>
          <p className='text-sm text-content-secondary mt-0.5'>Manage users, roles, and invitation links.</p>
        </div>
        <button
          onClick={handleInviteUserTrigger}
          className='btn-coral flex items-center gap-2 px-5 h-11 shadow-md shadow-primary/20 hover:shadow-lg hover:shadow-primary/30 transition-all hover:-translate-y-0.5'
        >
      ...
      {/* ── Tabs ── */}
      <div className='flex items-center gap-1 p-1 bg-white border border-ui-border rounded-xl w-fit shadow-sm'>
        {([
        ...
        <div className='space-y-5'>
                  ...
                  <div key={driver._id} className='flex items-center justify-between p-4 bg-white rounded-xl border border-amber-100 shadow-sm'>
                    <div className='flex items-center gap-3 min-w-0'>
                      ...
                      <div className='min-w-0'>
                        <div className='font-bold text-content-primary text-sm truncate'>{driver.name}</div>
                        <div className='text-xs text-content-secondary truncate'>{driver.email}</div>
                      </div>
                    </div>
          ...
          <div className='flex flex-col sm:flex-row gap-3'>
            <div className='relative flex-1'>
              <Search className='absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-content-secondary/50' />
              <input
                type='text'
            ...
            <div className='flex gap-2'>
              <div className='relative'>
                <Filter className='absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-content-secondary/50 pointer-events-none' />
                <select
                  value={roleFilter}
              ...
              <button onClick={loadUsers} className='btn-coral px-6 h-11 shrink-0'>Search</button>
            </div>
          </div>
              ...
              <>
                <div className='overflow-x-auto'>
                  <table className='w-full'>
                    <thead>
                      <tr className='border-b border-ui-border bg-app-bg/60'>
                        <th className='text-left px-5 py-3.5 text-[11px] font-bold text-content-secondary uppercase tracking-wider'>User</th>
                        <th className='text-center px-5 py-3.5 text-[11px] font-bold text-content-secondary uppercase tracking-wider'>Role</th>
                        <th className='text-left px-5 py-3.5 text-[11px] font-bold text-content-secondary uppercase tracking-wider'>Status</th>
                        <th className='text-left px-5 py-3.5 text-[11px] font-bold text-content-secondary uppercase tracking-wider hidden md:table-cell'>Joined</th>
                        <th className='text-right px-5 py-3.5 text-[11px] font-bold text-content-secondary uppercase tracking-wider'>Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {users.map((user, i) => (
                        <tr
                          key={user._id}
                          className={`group border-b border-ui-border/60 last:border-0 hover:bg-app-bg/40 transition-colors ${i % 2 === 0 ? '' : 'bg-app-bg/20'}`}
                        >
                          <td className='px-5 py-4'>
                            <div className='flex items-center gap-3'>
                              <UserAvatar name={user.name} avatar={user.avatar} size='md' />
                              <div className='min-w-0'>
                                <div className='font-semibold text-content-primary text-sm truncate'>{user.name}</div>
                          ...
                          <td className='px-5 py-4 text-center'>
                            <div className='relative inline-block'>
                              <button
                                ...
                                className={`flex items-center gap-1 transition-all ${user.role !== 'admin' ? 'hover:opacity-80 cursor-pointer' : 'cursor-default'} ${openRoleMenu === user._id ? 'ring-2 ring-primary/30 ring-offset-1 rounded-md' : ''}`}
                              >
                                {roleBadge(user.role)}
                                {user.role !== 'admin' && <Settings2 className='w-3 h-3 text-content-secondary/40' />}
                              </button>
                              {openRoleMenu === user._id && (
                                <div className='absolute top-full left-1/2 -translate-x-1/2 mt-2 w-28 bg-white border border-ui-border rounded-xl shadow-xl z-50 overflow-hidden py-1'>
                                  {['rider', 'driver'].filter(r => r !== user.role).map(role => (
                                    <button
                                      key={role}
                                      onClick={() => handleRoleChange(user._id, role)}
                                      className='w-full text-left px-3 py-2 text-xs font-bold text-content-secondary hover:bg-app-bg hover:text-primary transition-colors capitalize'
                                    >
                                      → {role}
                                    </button>
                                  ))}
                          ...
                          <td className='px-5 py-4 text-xs text-content-secondary hidden md:table-cell'>
                          ...
                          <td className='px-5 py-4 text-right'>
                            {user.role !== 'admin' && (
                              <button
                                ...
                                className='p-2 text-content-secondary hover:text-red-600 hover:bg-red-50 rounded-lg transition-all opacity-0 group-hover:opacity-100'
                                title='Delete user'
                              >
                ...
                {totalUserPages > 1 && (
                  <div className='flex items-center justify-between px-5 py-3.5 border-t border-ui-border bg-app-bg/30'>
                    ...
                    <span className='text-sm text-content-secondary font-medium'>Page {userPage} / {totalUserPages}</span>
      ...
      {activeTab === 'invitations' && (
        <div className='space-y-5'>
          ...
          <div className='grid grid-cols-2 lg:grid-cols-4 gap-3'>
            {[
              { label: 'Total', count: inviteStats.total, color: 'border-content-secondary/20', numColor: 'text-content-primary' },
              ...
              <div key={s.label} className={`bg-white rounded-2xl border p-4 shadow-sm ${s.color}`}>
                <div className='text-[10px] uppercase font-bold tracking-widest text-content-secondary/60 mb-1'>{s.label}</div>
                <div className={`text-3xl font-display font-bold ${s.numColor}`}>{s.count}</div>
              </div>
            ))}
          </div>
          ...
          <div className='flex flex-col sm:flex-row items-start sm:items-center gap-3 justify-between'>
                ...
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-lg text-xs font-bold border transition-all ${
                    inviteFilter === s
                      ...
                      : 'bg-white text-content-secondary border-ui-border hover:border-primary/40 hover:text-primary'
                  }`}
                >
            ...
            <button
              ...
              className='btn-coral flex items-center gap-2 px-5 h-9 text-sm shrink-0'
            >
            ...
            {loadingInvites ? (
              ...
              <div className='overflow-x-auto'>
                <table className='w-full'>
                  <thead>
                    <tr className='border-b border-ui-border bg-app-bg/60'>
                      <th className='text-left px-5 py-3.5 text-[11px] font-bold text-content-secondary uppercase tracking-wider'>Recipient</th>
                      <th className='text-center px-5 py-3.5 text-[11px] font-bold text-content-secondary uppercase tracking-wider'>Invited As</th>
                      <th className='text-left px-5 py-3.5 text-[11px] font-bold text-content-secondary uppercase tracking-wider'>Status</th>
                      <th className='text-left px-5 py-3.5 text-[11px] font-bold text-content-secondary uppercase tracking-wider hidden md:table-cell'>Expires</th>
                      <th className='text-right px-5 py-3.5 text-[11px] font-bold text-content-secondary uppercase tracking-wider'>Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {invitations.map((inv, i) => (
                      <tr
                        key={inv._id}
                        className={`group border-b border-ui-border/60 last:border-0 transition-colors ${
                          inv.status === 'revoked' ? 'bg-red-50/30 opacity-70' : i % 2 === 0 ? 'hover:bg-app-bg/40' : 'bg-app-bg/20 hover:bg-app-bg/40'
                        }`}
                      >
                        <td className='px-5 py-4'>
                          <div className='flex items-center gap-3'>
                            <div className='w-8 h-8 rounded-full bg-app-bg border border-ui-border flex items-center justify-center shrink-0'>
                              <Mail className='w-3.5 h-3.5 text-content-secondary/50' />
                            </div>
                            <span className='text-sm font-semibold text-content-primary'>{inv.email}</span>
                          </div>
                        </td>
                        <td className='px-5 py-4 text-center'>
                          ...
                          {inv.status === 'pending' && (
                            <div className='flex justify-end gap-1'>
                              <button
                                ...
                                className='p-2 text-content-secondary hover:text-primary hover:bg-primary/5 rounded-lg transition-all'
                                title='Resend'
                              >
                              ...
                              <button
                                ...
                                className='p-2 text-content-secondary hover:text-red-600 hover:bg-red-50 rounded-lg transition-all'
                                title='Revoke'
                              >
        ...
        <div className='fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm'>
          <div className='bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden'>
            ...
            <div className='flex items-center justify-between px-6 py-5 border-b border-ui-border'>
              <div>
                <h3 className='text-lg font-bold text-content-primary'>Invite Users</h3>
                <p className='text-xs text-content-secondary mt-0.5'>Send registration links to one or many users.</p>
              </div>
              <button
                onClick={() => setIsInviteModalOpen(false)}
                className='p-2 hover:bg-app-bg rounded-xl transition-colors text-content-secondary'
              >
                <X className='w-5 h-5' />
              </button>
            </div>
            ...
            <form onSubmit={handleSendInvitation} className='p-6 space-y-5'>
              ...
              <div>
                ...
                <div className='relative'>
                  <textarea
                    ...
                    autoFocus
                  />
                  <div className='absolute bottom-2 right-3 text-[10px] text-content-secondary/40 font-medium'>
              ...
              {/* Role selector */}
              <div>
                <label className='block text-xs font-bold text-content-secondary uppercase tracking-wider mb-3'>
                  Invite As
                </label>
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center gap-2 p-4 rounded-xl border-2 text-left transition-all ${
                        inviteRole === opt.value
                          ...
                          : 'border-ui-border text-content-secondary hover:border-primary/40 hover:text-content-primary'
                      }`}
                    >
              ...
              <button
                ...
                className='btn-coral w-full h-12 text-sm font-bold shadow-md shadow-primary/20 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2'
              >
                {isSendingInvite ? (
        ...
        <div className='fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm'>
          <div className='bg-white w-full max-w-sm rounded-2xl shadow-2xl p-6'>
            <div className='flex items-center gap-3 mb-4'>
              ...
              <h3 className='text-lg font-bold text-content-primary'>{confirmModal.title}</h3>
            </div>
            ...
            <div className='flex gap-3'>
              ...
              <button
                ...
                className='btn-coral flex-1 !bg-red-600 !hover:bg-red-700'
              >
