...
export default function AdminManageBuses() {
  ...
  const loadData = async () => {
    try {
      ...
      toast({ type: "error", message: "Failed to load fleet data" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (data: BusFormData) => {
    ...
    try {
      if (editBus) {
        await busesApi.update(editBus._id, {
          name: data.name,
      ...
      toast({ type: "error", message: "Failed to save bus" });
    } finally {
    ...
    <>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 40, flexWrap: "wrap" }}>
              {[
                ...
                {
                  value: buses.length - assignedBuses.length,
                  label: "Unassigned",
                  color: "#64748b",
                },
      ...
      {buses.length === 0 ? (
        ...
        >
          ...
          >
            Add your first bus to start managing your transit fleet.
          </p>
          <button
            onClick={() => {
              setEditBus(null);
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                ...
                >
                  {["Bus", "Route", "Driver", "Capacity", "Status", ""].map(
                    (h) => (
                      <th
                        key={h}
                        style={{
              ...
              <tbody>
                {buses.map((bus, i) => {
                  const routeObj =
                    bus.routeId && typeof bus.routeId === "object"
                      ? (bus.routeId as { name: string })
                      : null;
                  const routeName =
                    routeObj?.name ??
                    routes.find((r) => r._id === bus.routeId)?.name;
                  const driverObj =
                    bus.driverId && typeof bus.driverId === "object"
                      ? (bus.driverId as { name: string })
                      : null;
                  const driverName =
                    driverObj?.name ??
                  ...
                  return (
                    <tr
                      ...
                      style={{
                        borderBottom:
                          i < buses.length - 1 ? "1px solid #f8fafc" : "none",
                      }}
                    >
                      {/* Bus name */}
                        ...
                        >
                          ...
                          >
                            <div
                              style={{
                                height: "100%",
                                width: `${Math.min((bus.capacity / 60) * 100, 100)}%`,
                                background: `linear-gradient(90deg, ${C.indigo}, ${C.sky})`,
                                borderRadius: 3,
                              }}
                            />
                        ...
                        >
                          <span
                            style={{
                              ...
                              height: 6,
                              borderRadius: "50%",
                              background: bus.isActive ? C.indigo : "#cbd5e1",
                              boxShadow: bus.isActive
                                ? `0 0 4px ${C.indigo}`
                                : "none",
                            }}
                          />
                      ...
                      <td style={{ padding: "13px 18px" }}>
                        ...
                        >
                          ...
                          <button
                            onClick={() => setConfirmDeleteId(bus._id)}
                            className="w-8 h-8 flex items-center justify-center rounded-lg transition-colors"
                            style={{
                              color: "#94a3b8",
                              background: "transparent",
      ...
      <ConfirmDialog
        ...
        title="Remove bus?"
        description="This bus will be permanently removed."
        confirmLabel="Remove"
        variant="danger"
      />
    </>
...
function BusForm({
  ...
  const [form, setForm] = useState<BusFormData>({
    ...
    routeId:
      (bus?.routeId && typeof bus.routeId === "object"
        ...
        : (bus?.routeId as string)) || "",
    driverId:
      (bus?.driverId && typeof bus.driverId === "object"
        ? (bus.driverId as { _id: string })._id
        : (bus?.driverId as string)) || "",
  ...
  return (
    ...
    >
      <div className="bus-form-grid" style={{ gap: 12 }}>
        <div>
